      ...
      <el-table v-if="tableData.length"
        ...
        <el-table-column prop="type"
          ...
          <template slot-scope="scope">
            <span>{{ scope.row.type == '1' ? 'Dish Type' : 'Set Meal Type' }}</span>
          </template>
        </el-table-column>
...
export default class extends Vue {
      ...
      name: [
        {
          ...
          validator: (rule: any, value: string, callback: Function) => {
            ...
            if (!value) {
              callback(new Error(this.classData.title + 'can\'t be empty'))
  ...
  private addClass(st: any) {
    if (st == 'class') {
      this.classData.title = 'Add Dish Type'
      this.type = '1'
    } else {
      this.classData.title = 'Add Set Meal Type'
      this.type = '2'
    }
    this.action = 'add'
    this.classData.name = ''
  ...
  private statusHandle(row: any) {
    ...
    this.$confirm('Do you confirm to modify the status of this category?', 'Hint', {
      ...
      enableOrDisableEmployee({ id: this.id, status: !this.status ? 1 : 0 })
        .then(res => {
          if (String(res.status) === '200') {
            this.$message.success('Category is modified successfully!')
            this.init()
          }
        })
        .catch(err => {
  ...
  private deleteHandle(id: any) {
    this.$confirm('此操作将永久删除该分类，是否继续？', 'Confirm', {
